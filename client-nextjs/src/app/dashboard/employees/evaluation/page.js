...
export default function PerformanceEvaluationPage() {
  ...
  const getGoalStatusBadge = (status) => {
    switch (status) {
      ...
      case 'partially':
        return <Badge className="bg-yellow-100 text-yellow-800">부분달성</Badge>
      case 'in_progress':
  ...
  const calculateStats = () => {
    const completed = evaluationData.filter(e => e.status === 'completed').length
    const inProgress = evaluationData.filter(e => e.status === 'in_progress').length
    const pending = evaluationData.filter(e => e.status === 'pending').length
    const averageScore = evaluationData
            ...
            {filteredEvaluations.map((evaluation) => (
              <div key={evaluation.id} className="border rounded-lg p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold">{evaluation.employee}</h3>
                    <p className="text-gray-600">{evaluation.department} / {evaluation.position}</p>
                    <p className="text-sm text-gray-500">평가자: {evaluation.evaluator}</p>
                  </div>
                    ...
                    {evaluation.overallScore && (
                      <div className="mt-2">
                        ...
                        <div className="flex space-x-1 justify-end">
                          {renderStarRating(Math.round(evaluation.overallScore))}
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                      <div className="text-center">
                        ...
                        <p className={`font-semibold ${getScoreColor(evaluation.categories.performance)}`}>
                          {evaluation.categories.performance}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-gray-600">소통능력</p>
                        <p className={`font-semibold ${getScoreColor(evaluation.categories.communication)}`}>
                          {evaluation.categories.communication}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-gray-600">리더십</p>
                        <p className={`font-semibold ${getScoreColor(evaluation.categories.leadership)}`}>
                          {evaluation.categories.leadership}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-gray-600">혁신성</p>
                        <p className={`font-semibold ${getScoreColor(evaluation.categories.innovation)}`}>
                          {evaluation.categories.innovation}
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-gray-600">팀워크</p>
                        <p className={`font-semibold ${getScoreColor(evaluation.categories.teamwork)}`}>
                          ...
                          {evaluation.goals.map((goal, index) => (
                            <div key={index} className="flex items-center justify-between">
                              ...
                              <div className="ml-4 text-right">
                                <p className="text-sm font-semibold">{goal.achievement}%</p>
                    ...
                    {evaluation.feedback && (
                      <div className="mb-4">
                        ...
                        <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{evaluation.feedback}</p>
                      </div>
                    )}
                    ...
                    {evaluation.nextGoals && (
                      <div className="mb-4">
                        ...
                        <p className="text-sm text-gray-700 bg-blue-50 p-3 rounded">{evaluation.nextGoals}</p>
                      </div>
                    )}
                  </>
                )}
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  <div className="text-sm text-gray-500">
                    ...
                    {evaluation.completedDate && ` | 완료일: ${evaluation.completedDate}`}
                  </div>
                  <div className="flex space-x-2">
                    <Button variant="outline" size="sm" onClick={() => setSelectedEmployee(evaluation)}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div>
                    ...
                    <select className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                      ...
                      <option value="Nguyen Van A">Nguyen Van A (마케팅팀)</option>
              ...
              {/* 평가 카테고리 */}
              <div>
                <Label className="text-base font-semibold">평가 카테고리 (1-5점)</Label>
              ...
              {/* 목표 설정 */}
              <div>
                <Label className="text-base font-semibold">목표 및 달성도</Label>
                <div className="space-y-3 mt-2">
                  {[1, 2, 3].map((index) => (
                    <div key={index} className="grid grid-cols-3 gap-2 p-3 border rounded">
                      <Input placeholder={`목표 ${index}`} />
                      <Input type="number" placeholder="달성률 (%)" min="0" max="100" />
