...
const ObjectiveCard = ({
  ...
  const handleNumericUpdate = (krId, index) => {
    if (numericEdit[krId]?.dirty) {
      onUpdateKeyResultValue(index, 'currentValue', Number(numericEdit[krId].value));
      setNumericEdit((prev) => ({ ...prev, [krId]: { value: numericEdit[krId].value, dirty: false } }));
    }
  };
            ...
            {isEditing && (
              <div className="flex gap-2 mb-4">
                <Button
                  ...
                  disabled={editingObjectiveData.keyResults?.some(kr => kr.isNew)}
                >
                  + {t('수치 기반 KR 추가')}
                </Button>
                <Button
                    ...
                    <div className="relative bg-white rounded-lg p-4">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <span className="font-semibold text-blue-700 text-base">
                          {kr.type === 'numeric' ? t('수치 기반 핵심 결과 추가') : t('체크리스트 기반 핵심 결과 추가')}
                        </span>
                        <button
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-3 gap-3 mb-2">
                            <Input
                              ...
                              onChange={e => onUpdateKeyResultValue(index, 'currentValue', parseFloat(e.target.value) || 0)}
                              placeholder={t('현재 값')}
                            />
                            <Input
                              type="number"
                              value={kr.target || ''}
                              onChange={e => onUpdateKeyResultValue(index, 'target', e.target.value)}
                              placeholder={t('목표 값')}
                            />
                            <Input
                              value={kr.unit || ''}
                              onChange={e => onUpdateKeyResultValue(index, 'unit', e.target.value)}
                              placeholder={t('단위')}
                            />
                          </div>
                        </>
                        ...
                        <>
                          ...
                          {(kr.checklist || []).map((item, itemIndex) => (
                            <div key={itemIndex} className="flex items-center gap-2 mb-1">
                              <Input
                                value={item.text}
                                onChange={e => onUpdateChecklistItemInEdit(index, itemIndex, e.target.value)}
                                placeholder={t('체크리스트 항목을 입력하세요')}
                              />
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => onRemoveChecklistItemInEdit(index, itemIndex)}
                                className="text-red-500 hover:text-red-700"
                              >
                                <Trash2 className="w-4 h-4" />
                              </Button>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    >
                      {kr.type === "numeric" ? t('수치 기반') : t('체크리스트 기반')}
                    </Badge>
                  </div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div>
                            ...
                            <div className="flex gap-2 items-center">
                              <Input
                                type="number"
                                value={numericEdit[kr.id]?.value ?? (kr.current || kr.currentValue || "")}
                                onChange={(e) => handleNumericInput(kr.id, e.target.value)}
                                className="w-full"
                                placeholder="0"
                              />
                              <Button
                                ...
                                onClick={() => handleNumericUpdate(kr.id, index)}
                              >
                                {t('업데이트')}
                              </Button>
                            </div>
                        ...
                        <>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-600 dark:text-gray-400">
                              {t('현재:')} {(kr.currentValue || 0).toLocaleString()}
                              {kr.unit} / {t('목표:')} {" "}
