...
export default function CustomersPage() {
  ...
  const getGradeColor = (grade) => {
    switch (grade) {
      case "A": return "bg-gradient-to-r from-red-100 to-red-200 text-red-800 dark:from-red-900 dark:to-red-800 dark:text-red-300";
      case "B": return "bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 dark:from-blue-900 dark:to-blue-800 dark:text-blue-300";
      case "C": return "bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 dark:from-gray-700 dark:to-gray-600 dark:text-gray-300";
      default: return "bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 dark:from-gray-700 dark:to-gray-600 dark:text-gray-300";
    }
  };
      ...
      <div className="space-y-8 animate-fadeIn">
        ...
        <div className="relative bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-purple-500/10 
                         dark:from-blue-400/10 dark:to-purple-400/10"></div>
        ...
        <Card className="transform transition-all duration-500 hover:shadow-2xl
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <Input
                ...
                className="md:col-span-2 transition-all duration-300 focus:scale-105 focus:shadow-lg
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { title: t('customers.total'), value: customers.length, color: "from-blue-500 to-cyan-500", icon: "👥" },
            { title: t('customers.gradeA'), value: customers.filter(c => c.grade === "A").length, color: "from-red-500 to-pink-500", icon: "⭐" },
            { title: t('customers.gradeB'), value: customers.filter(c => c.grade === "B").length, color: "from-blue-500 to-indigo-500", icon: "🏆" },
            { title: t('customers.gradeC'), value: customers.filter(c => c.grade === "C").length, color: "from-gray-500 to-slate-500", icon: "📊" }
          ].map((stat, index) => (
        ...
        <div className="grid gap-6">
          {currentCustomers.map((customer, index) => {
            const customerName = customer?.name || customer?.companyName || "Unknown Company";
            ...
            return (
              <Card key={customer.id} 
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/5 
                               group-hover:from-blue-500/10 group-hover:to-purple-500/10 transition-all duration-500"></div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-4">
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white font-bold text-lg
                                     ...
                                     ${customer.grade === 'A' ? 'bg-gradient-to-br from-red-500 to-red-600' :
                                       customer.grade === 'B' ? 'bg-gradient-to-br from-blue-500 to-blue-600' :
                                       'bg-gradient-to-br from-gray-500 to-gray-600'}`}>
                        {customerName.charAt(0)}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <CardTitle className="text-xl text-gray-900 dark:text-white group-hover:text-blue-600 
                                               dark:group-hover:text-blue-400 transition-colors duration-300">
                            {customerName}
                          </CardTitle>
                          <span className={`px-3 py-1 rounded-full text-xs font-medium shadow-sm ${getGradeColor(customer.grade)}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    {customer.email && (
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50
                        ...
                        <svg className="w-4 h-4 text-gray-500 group-hover:text-blue-500 transition-colors duration-300" 
                    ...
                    {customer.phone && (
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50
                        ...
                        <svg className="w-4 h-4 text-gray-500 group-hover:text-green-500 transition-colors duration-300" 
                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                    ...
                    {customer.address && (
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50
                        ...
                        <svg className="w-4 h-4 text-gray-500 group-hover:text-purple-500 transition-colors duration-300" 
                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                                d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                        <span className="text-sm text-gray-600 dark:text-gray-400 truncate">{customer.address}</span>
                      </div>
                    )}
                  </div>
          ...
          <Card className="text-center py-16 bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg border-0 shadow-xl">
            <CardContent>
              ...
              <Button 
                onClick={() => router.push("/dashboard/customers/add")}
                className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600
                         transition-all duration-300 transform hover:scale-105 hover:shadow-lg px-8 py-3">
