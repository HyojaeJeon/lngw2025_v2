  ...
  Query: {
    ...
    checkProductCode: async (parent, { code }, { user, lang }) => {
      try {
        ...
        if (existingProduct) {
          return {
            isAvailable: false,
            message: lang === "ko" ? "이미 사용 중인 상품 코드입니다." : lang === "en" ? "Product code is already in use." : "Mã sản phẩm đã được sử dụng.",
          };
        }

        return {
          isAvailable: true,
          message: lang === "ko" ? "사용 가능한 상품 코드입니다." : lang === "en" ? "Product code is available." : "Mã sản phẩm có thể sử dụng.",
        };
      } catch (error) {
        return {
          isAvailable: false,
          message: lang === "ko" ? "코드 확인 중 오류가 발생했습니다." : lang === "en" ? "Error occurred while checking code." : "Lỗi xảy ra khi kiểm tra mã.",
        };
      }
    },
    ...
    lowStockProducts: async (parent, { limit = 20 }, { user, lang }) => {
      try {
        ...
        const products = await Product.findAll({
          where: {
            [Op.and]: [models.sequelize.literal("currentStock <= minStock"), { isActive: true }],
          },
          include: [
            {
              model: Category,
  ...
  Mutation: {
    ...
    createProduct: async (parent, { input }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품이 성공적으로 생성되었습니다." : lang === "en" ? "Product created successfully." : "Sản phẩm đã được tạo thành công.",
    ...
    updateProduct: async (parent, { id, input }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품이 성공적으로 수정되었습니다." : lang === "en" ? "Product updated successfully." : "Sản phẩm đã được cập nhật thành công.",
    ...
    deleteProduct: async (parent, { id }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품이 성공적으로 삭제되었습니다." : lang === "en" ? "Product deleted successfully." : "Sản phẩm đã được xóa thành công.",
        };
      } catch (error) {
        return handleDatabaseError(error, lang);
      }
    ...
    restoreProduct: async (parent, { id }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품이 성공적으로 복원되었습니다." : lang === "en" ? "Product restored successfully." : "Sản phẩm đã được khôi phục thành công.",
          product,
        };
      } catch (error) {
        return handleDatabaseError(error, lang);
    ...
    updateProductStatus: async (parent, { id, status }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품 상태가 성공적으로 변경되었습니다." : lang === "en" ? "Product status updated successfully." : "Trạng thái sản phẩm đã được cập nhật thành công.",
          product,
        };
      } catch (error) {
        return handleDatabaseError(error, lang);
    ...
    updateStock: async (parent, { id, quantity, type }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "재고가 성공적으로 업데이트되었습니다." : lang === "en" ? "Stock updated successfully." : "Tồn kho đã được cập nhật thành công.",
          product,
        };
      } catch (error) {
        return handleDatabaseError(error, lang);
    ...
    updateProductOrder: async (parent, { id, sortOrder }, { user, lang }) => {
      try {
        ...
        return {
          success: true,
          message: lang === "ko" ? "상품 순서가 성공적으로 변경되었습니다." : lang === "en" ? "Product order updated successfully." : "Thứ tự sản phẩm đã được cập nhật thành công.",
          product,
        };
      } catch (error) {
        return handleDatabaseError(error, lang);
